...
Systeme sind chaotisch, wenn sie in labile Zustände in ihren Freiheitsgraden gelagen können.
...
Interessant ist, dass die Systeme eigentlich deterministisch sind und mithilfe der Startbedingungen der weitere Verlauf vollständig berechnet werden könnte.
Messfehler und Ungenauigkeiten in der Berechnung führen allerdings zu Fehlern, die sich in labilen Zuständen sehr stark auswirken.
Aus diesem Grund weicht eine Simulation meist schnell von der Realität ab.
...
Hier sind die Formeln für die Berechnung der Energien zu sehen, die später auch in der Simulation gezeigt werden, und die durch die Konstanten $k_1$ bis $k_5$ verallgemeinert sind. $T$ und $V$ sind die kinetische und die potentielle Energie:
...
Daraus ergeben sich diese Euler-Lagrange-Gleichungen:
...
Nach verschiedenen Umformungsschritten lauten die Differentialgleichungen wie folgt:
\mathematik
...
\subsection{Untersuchung auf Frequenzverdopplung}
...
Der Winkel des zweiten Pendelarms wird dabei immer mit der doppelten Größe initialisert: $\phi_2 = 2\phi_1$.
In Abbildung \TODO ist ein Diagramm zu sehen, auf dem auf der vertikalen Achse die Positionen der Maxima aus der Fourieranalyse für die ersten sechs Sekunden der Simulation aufgetragen sind. Von links nach rechts wird $\phi_1$ linear von $0\;rad$ bis $\pi/4\;rad$ vergrößert.
