...
Jedes Bild hat eine Auflösung von 300 mal 300 Pixeln auf denen auf der waagerechten Achse nach rechts $\phi_1$ und nach oben $\phi_2$ aufgetragen ist.
Der grüne Farbanteil codiert die Anzahl der Übeschläge nach links, der blaue Anteil die Überschläge nach rechts.
Weil die Startbedingungen nur auf der einen Seite des Pendels (für Winkel größer Null) durchgeführt wurden, überwiegt die Anzahl der Überschläge nach links.

In Bild \ref{fig:fraktale_gesamt} ist $\phi_1$ im Intervall $[0; \pi/2]$ und nach oben ist $\phi_2$ im Intervall $[0; \pi]$ zu sehen.
Im schwarzen Bereich ist einfach nicht genug Energie im System für einen Überschlag.
Interessant ist der blaue Bereich unten rechts, in dem überwiegend Überschläge nach rechts stattfinden, und der Bereich in der Mitte oben, in dem die Struktur zerfasert.
Zwei Ausschnitte aus diesem letzten Bereich sind in Abbildung \ref{fig:fraktale_c} und \ref{fig:fraktale_d} gezeigt.

In der Vergrößerung ist sehr schön ein fraktales Muster zu erkennen, das typisch für chaotische Systeme ist, wie zum Beispiel dem magnetischen Pendel. \citep{wikimagnetisch}
